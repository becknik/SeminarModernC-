...
std::string
readFromCache(int page)
{
    std::ifstream cacheFile(getCacheFileName(page));
    return std::string((std::istreambuf_iterator<char>(cacheFile)),
...
auto main() -> int
{
    ...
    for (const auto page : std::views::iota(1, 173)) {
        ...
        if (isCacheAvailable(page)) {
            ...
            std::string fullUrl = std::format(
                "https://jisho.org/api/v1/search/words?keyword=%23jlpt-n1&page={}",
                page);
            ...
            if (res != CURLE_OK) {
                std::cerr << "Error fetching data: " << curl_easy_strerror(res)
                          << std::endl;
    ...
    // std::ranges::sort(vocabList, {}, [](const JapVocab& vocab) {return
    // vocab.word;});
    ...
    for (auto i = 0; i < 10; ++i) {
        ...
        auto filteredList {
            vocabList | std::views::filter([](const auto& v) { return v.isCommon; }) | std::views::transform([](const auto& v) { return v.reading; }) | std::views::drop(20 * urlPageParameter) | std::views::take(20)
        };
